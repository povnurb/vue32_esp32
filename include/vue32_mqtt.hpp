...
String Json()
{
    ...
    jsonDoc["temp_cpu"] = String(round(TempCPUValue()), 0);
    jsonDoc["tempC"] = String(round(Temperatura()), 0);
    jsonDoc["humedad"] = String(round(Humedad()), 0);
    jsonDoc["tmin"] = String(round(tempMin()), 0);
    jsonDoc["tmax"] = String(round(tempMax()), 0);
    jsonDoc["prueba"] = prueba;
    jsonDoc["value"] = RELAY_STATUS;
...
void mqttLoop()
{
    ...
    { // si es true osea si esta habilitado
        // log("MQTT habilitado");
