...
bool contra = true; // habilita la contraseña  ***************************************************
...
void putRequestAlarmas(AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
{
    ...
    String bodyContent = GetBodyContent(data, len);
    log(bodyContent);
    StaticJsonDocument<2500> doc; // podriamos ver si jala reduciendo 2500
    ...
    if (error)
    {
        ...
        log("[ INFO:vue32_server.hpp ] Problemas con el tamaño de StaticDocument linea 804");
        return;
    };
    // -------------------------------------------------------------------
...
void InitServer()
{
    ...
    // -------------------------------------------------------------------
    server.on(
        "/api/alarmas", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL, putRequestAlarmas);
